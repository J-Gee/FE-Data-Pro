...
def batch_processor(params, data):
    ...
    for i in data[0]:
        ...
        for i in real_list:
            ...
            #print(col_name)
            current_file_df["calc % {}".format(col_name)] = (current_file_df[i] *100)/current_file_df["sum"]
        ...
        for col in current_file_df.columns:
            ...
            if col == "calc % H2" or col == "calc % O2":
                ...
                processed_output_dict.setdefault(("{} 2STD").format(col), []).append(current_file_df[("{}").format(col)].std() * 2)
                processed_output_dict.setdefault("{} umol".format(place_hol), []).append(avg_gas_umol)
                processed_output_dict.setdefault("{} umol/h".format(place_hol), []).append(avg_gas_umol/illu_time)
